...
from core.models import proper_hnn_loss, port_hamiltonian_loss
...
def train_model(
    ...
    """
    Train model with Adam + early stopping on val reconstruction MSE.

    Checkpoint saved to {checkpoint_dir}/{model_name}_best.pt.
    Returns (history dict, best_val_mse).
    """
...
# ──────────────────────────────────────────────────────────────────────────────
# Short wrapper for hparam search
# ──────────────────────────────────────────────────────────────────────────────

def quick_train(model, loaders, device, lambda_phys=0.0, lr=1e-3,
                max_epochs=50, patience=10):
    """Fast training run for hyperparameter search. No checkpointing or logging."""
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    best_val_mse = math.inf
    epochs_no_improve = 0
