...
import pycbc.psd
import torch
from pycbc.filter import matched_filter, sigma, highpass, lowpass
...
F_HIGH          = SAMPLE_RATE / 2  # Hz — Nyquist frequency (low-pass filter)
...
def load_hdf5_chunk(
    ...
    Args:
        ...
        rng:             Random generator to allow deterministic sampling
    ...
    with h5py.File(filepath, 'r') as f:
        ...
        if chunk_start_idx is None:
            chunk_start_idx = rng.randint(buffer, max_start)
...
def get_random_chunk(
    ...
    Args:
        ...
        rng:      Random generator for deterministic sampling
        files:    Optional precomputed list of HDF5 filenames
    ...
    filepath = os.path.join(data_dir, rng.choice(files))
    return load_hdf5_chunk(filepath, rng=rng)
...
def inject_signal(
    ...
    The injection time is randomized within the central 1.5s window so the
    model does not learn a fixed arrival time.
    ...
    Args:
        ...
        rng:           Random generator for repeatable injection timing

    Returns:
        injected:     2.5s noisy strain with signal injected
        clean:        2.5s clean signal only (zero-padded to window)
        achieved_snr: Target injection SNR
    """
    if rng is None:
        rng = random

    # The last 2.5s of the noise segment is used for injections
    n_inject        = int(INJECT_DURATION * SAMPLE_RATE)
    injection_noise = noise_segment[len(noise_segment) - n_inject:]

    # Scale waveform to achieve target SNR (using the supplied PSD)
    ...
    waveform_arr = np.array((waveform * scale).data, dtype=np.float64)

    # Randomize injection time within the central 1.5s portion of the 2.5s window.
    # This makes signal placement stochastic while ensuring the final cropped
    # 1.5s segment (after whitening) can contain the waveform in different positions.
    n_crop   = int(CROP * SAMPLE_RATE)
    start_ix = rng.randint(n_crop, n_crop + N_SAMPLES - 1)

    # Build clean signal segment with injection placed at a random offset.
    clean_arr = np.zeros(n_inject, dtype=np.float64)
    end_ix = min(n_inject, start_ix + len(waveform_arr))
    clean_arr[start_ix:end_ix] = waveform_arr[: end_ix - start_ix]

    # Inject into noise (using array arithmetic for deterministic alignment)
    injected = TimeSeries(
        injection_noise.data + clean_arr,
        delta_t=injection_noise.delta_t,
        epoch=injection_noise.start_time
    )

    clean = TimeSeries(clean_arr, delta_t=injected.delta_t, epoch=injected.start_time)
...
def whiten_and_crop(
    ...
    """Whiten a strain segment and return the central 1.5s crop.

    Whitening is done using :func:`pycbc.filter.whiten`, which performs
    a frequency-domain normalization by the PSD and applies a window
    taper to reduce spectral leakage.

    A low-pass filter is applied before whitening to avoid amplifying
    high-frequency components above the Nyquist limit (which are not
    physically meaningful and can be dominated by numerical noise).
    ...
    # Bandlimit before whitening (avoid amplifying high-frequency noise)
    ...
    strain_fd = bandpassed.to_frequencyseries()
    psd_interp = interpolate(psd, strain_fd.delta_f)

    # Whiten in frequency domain (avoid requiring pycbc.filter.whiten)
    whitened_fd = strain_fd / np.sqrt(psd_interp)
    whitened    = whitened_fd.to_timeseries()

    # Crop 0.5s from each edge to remove filter transients
    n_crop  = int(CROP * SAMPLE_RATE)
...
def generate_sample(
    ...
    """Full pipeline for one training sample: noise → PSD → waveform → inject → whiten → crop.
    ...
    if mass1 is None:
        mass1 = rng.uniform(10, 80)
    if mass2 is None:
        mass2 = rng.uniform(10, mass1)
    if target_snr is None:
        target_snr = rng.uniform(12, 100)

    noise    = get_random_chunk(data_dir, rng=rng, files=files)
    psd      = estimate_psd(noise)
    waveform = generate_bbh_waveform(mass1, mass2)

    injected, clean_signal, achieved_snr = inject_signal(
        noise, waveform, psd, target_snr, rng=rng
    )
...
class GravitationalWaveDataset(Dataset):
    ...
    Samples are generated on-demand in ``__getitem__`` to avoid storing
    large numbers of precomputed tensors in memory.
    ...
    def __init__(
        ...
        # Pre-fetch the list of eligible HDF5 files so that __getitem__ can be fast.
        self._files = [f for f in os.listdir(data_dir) if f.endswith('.hdf5')]
    ...
    def __getitem__(self, idx):
        ...
        target_snr = rng.uniform(*self.snr_range)
        ...
        return {
            "noisy_strain": torch.tensor(sample["noisy_strain"]).unsqueeze(0),  # [1, 3072]
            "clean_strain": torch.tensor(sample["clean_strain"]).unsqueeze(0),  # [1, 3072]
...
def matched_filter_benchmark(
    ...
    The function will sample fresh random noise chunks from ``data_dir``.
    If ``data_dir`` is not provided, it falls back to the global ``DATA_DIR``
    defined in the script's main section.
    """
    from pycbc.psd import interpolate as psd_interpolate
    ...
    if data_dir is None:
        ...
        if data_dir is None:
            raise ValueError("matched_filter_benchmark requires data_dir when called from outside __main__")

    rng = random.Random(seed) if seed is not None else random

    snr_values = []

    # Preload file list once for efficiency
    files = [f for f in os.listdir(data_dir) if f.endswith('.hdf5')]

    for _ in range(n_trials):
        # Fresh noise chunk each trial
        trial_noise = get_random_chunk(data_dir, rng=rng, files=files)
        ...
        injected, clean, _ = inject_signal(
            trial_noise, trial_waveform, trial_psd, target_snr, rng=rng
        )
        ...
        psd_for_template = interpolate(trial_psd, template.delta_f)
        ...
        print(f"sigma of padded: {sigma(padded, psd=psd_for_template, low_frequency_cutoff=F_LOWER):.2f}")
